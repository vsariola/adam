...
{  
    ...
    vec3 o = vec3(sin(beat*0.2)-15.0,cos(beat*0.2)+10.,cos(beat*0.2)*10.-10.);
    ...
    for (float d2=d;d2>0.;d2-=0.5) {              
        p -= 0.5*r;        
        col += (vec3(0.015)+(abs(p.x-p.y+20.)<40.*clamp((pattern-64.)/4.0,0.,1.)?pow(p.y/15.,4.):0.)*vec3(0.4,0.36,0.3) + screen(p.xy)*exp(p.z-25.) - col) * fogMap(p) * 0.03 * min(d2,0.5);        
    }
    ...
    for (int k = 0;k < 2;k++) {
        for (int j = 0;j < 1;j++) {   
            for (int i = 0;i < 20;i++) {                
                ...
                vec3 pos = dir * 4. + vec3(0.,10.,float((j+int(beat))%4-2)*10.);            
                dir.z += sin(beat)*1.0;            
                dir.xy += dir.yx * vec2(-1.,1.) * sin(beat);
                pos.x += 15.-float(k)*30.;            
                ...
                //b *= 1.0+pow(ld.x,20.);
                col += vec3(1.,1.,0.6)*b*(1.-fract(beat*3.));
            }
        }
    }
